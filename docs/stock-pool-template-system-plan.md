...
本文档用于规划一个独立的股票池模板系统。当前系统已明确新的职责边界：股票池模板只保存用户维护的股票集合；行情、指标和每日调度统一由主行情库与系统管理员运维看板管理。历史上的模板数据刷新 API 和任务查看 API 仅作为 legacy 运维入口保留，不再出现在股票池模板用户页面。
...
- 支持五类更新来源：`active_templates`、`template`、`symbols`、`all`、`main_universe`。当前 `all` 与 `main_universe` 等价，均只处理主股票池活跃股票；其余用于模板或手工股票的补数和刷新。
...
- 保留 legacy API：`POST /api/stock-pools/template/refresh`、`GET /api/stock-pools/jobs`、`GET /api/stock-pools/jobs/{job_id}`。这些接口只用于兼容旧脚本和人工排障，不再由股票池模板页面或系统管理员看板直接暴露。
- 数据刷新和任务查看仍为 admin-only legacy 能力。后端从登录态判断 admin 权限；普通用户可以管理自己的模板，但不能触发 Tushare 刷新或查看更新任务。
...
- 保存模板本身只保存模板和股票关系；保存时必须通过主股票池校验，不存在于主股票池的股票不允许写入模板。行情采集和指标计算由主行情库与统一调度负责，不再按模板维护独立数据刷新任务。
...
股票池模板系统用于让用户手工维护一组股票列表。模板只回答“这个模板包含哪些股票”；日线数据、指标和交易计划依赖的行情特征由主行情库统一存储和更新。后续其他模块可以从股票池模板读取股票范围，再从主行情库读取行情指标，而不是让模板拥有自己的数据源。

核心目标：

- 提供类似账户模板管理的前端页面，支持新建、复制、保存、删除股票池模板。
- 用户手工输入股票列表，系统不在本模块内做自动筛选。
- 保存模板时校验股票必须属于主股票池；主股票池不存在或已停用的股票不允许写入模板。
- 同一只股票在多个模板中复用主行情库的同一份日线和指标数据，不按模板重复采集、不重复存储。
- 每天晚上由统一调度更新主股票池股票的日线数据、指标和次日模拟账户订单，股票池模板不再拥有独立定时更新任务。
...
旧模块正在分阶段从 CSV 目录输入切换为股票池模板输入：

| 模块 | 当前输入 | 本阶段是否改动 |
| --- | --- | --- |
| 每日收盘选股 | 前端选择股票池模板；后端支持 `data_source=stock_pool` 读取 SQLite，并保留 `data_source=csv` 兼容 | 已接入 |
| 多账户模拟交易 | 账户模板 YAML 中的 `股票池` 节点，运行时从 `stock_daily_features` 读取信号、开盘成交价和收盘估值价 | 已接入，不再兼容旧 CSV 字段 |
| 批量回测 | 前端选择股票池模板；后端支持 `data_source=stock_pool` 读取 SQLite，并保留 `data_source=csv` 兼容 | 已接入 |
| 单股回测 | 股票代码 + 股票池 SQLite | 已接入，CSV/Excel 保留兼容 |
| 板块研究 | 板块研究 CSV、个股暴露 CSV、增强后的股票 CSV | 不改 |

本阶段先完成股票池模板系统、SQLite 数据结构和数据更新流程，并逐步把旧模块输入从 CSV 切换为股票池模板。

推荐后续接入顺序：

1. 批量回测先支持选择股票池模板读取 SQLite。
2. 每日收盘选股支持选择股票池模板。已完成。
3. 多账户模拟交易账户模板增加 `股票池` 字段并全面切到 SQLite；旧的 `处理后数据目录` 字段不再保留。已完成。
4. 单股回测已支持从 SQLite 查询单股历史，并保留旧 CSV/Excel 兼容入口。
...
- 更新状态：显示模板内股票的最新数据日期。
- 模板页不提供数据刷新、最近任务状态或任务明细入口；定时任务和失败重跑统一在系统管理员运维看板查看。

页面不提供自动筛选条件输入，不提供市值/主题/指数/行业筛选控件。普通用户和 admin 在该页面都只维护模板股票集合；后端 legacy 刷新和任务状态接口仍会拒绝非 admin 调用，但不再由该页面触发。
...
| `/api/stock-pools/template/refresh` | POST | legacy 手动触发模板范围数据更新；仅 admin；前端不再暴露 |
| `/api/stock-pools/jobs` | GET | legacy 查看更新任务列表；仅 admin；前端不再暴露 |
| `/api/stock-pools/jobs/{job_id}` | GET | legacy 查看任务明细；仅 admin；前端不再暴露 |

模板 API 从登录态读取当前用户。legacy 刷新和任务 API 仍会校验 admin 权限，非 admin 返回 403；`POST /api/stock-pools/template/refresh` 调用成功时返回 `legacy: true`，提醒调用方该入口不属于当前主流程。
...
- 保存模板后不直接阻塞式采集，也不创建模板级刷新任务；模板内股票的数据完整性由主股票池校验和主行情库统一调度保障。
...
- 页面已展示模板内每只股票最新数据日期。
- 模板页已移除 admin-only 数据运维区。系统管理员页面改为运维看板，查看核心任务状态、主股票池维护、任务运行记录和安全重跑；模板刷新/任务 API 仅作为 legacy 接口保留。
...
- 每日收盘选股已支持股票池模板输入：前端读取当前用户模板列表，后端复用批量回测的 SQLite 加载逻辑，并保留旧 CSV 兼容。
- 多账户模拟交易账户模板支持引用股票池模板，并且生成订单、执行本地日线成交、收盘估值和持有天数计算均从 `stock_daily_features` 读取。
- 单股回测已接入 SQLite；板块研究再逐步接入。
