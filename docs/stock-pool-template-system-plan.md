...
- 默认用户为 `505888`；后续接入账户名和密码登录后，模板按实际用户隔离。
...
| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `username` | TEXT | 用户名，默认 `505888` |
| `password_hash` | TEXT | 预留字段，后续登录系统使用 |
| `display_name` | TEXT | 展示名 |
| `created_at` | TEXT | 创建时间 |
| `updated_at` | TEXT | 更新时间 |

当前阶段可以只写入默认用户 `505888`，不实现登录。
...
当前无登录系统时，API 默认 `username=505888`。后续接入登录后，从 session 或 token 中读取用户名。
