...
本文档用于规划一个独立的股票池模板系统。当前第一阶段已经实现“模板 + 手工股票列表 + SQLite 基础表 + 前端管理页面”；第二阶段已经实现共享日线与指标入库、任务日志、任务表和 CLI/API 更新入口；第四阶段已先把批量回测和每日收盘选股接入股票池模板 SQLite。多账户模拟交易、单股回测和板块研究仍待后续逐步接入。
