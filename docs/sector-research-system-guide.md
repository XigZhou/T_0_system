...
| 个股暴露 | `sector_research/data/processed/stock_theme_exposure.csv` | 查看股票代码、股票名称、命中主题、命中板块、暴露分和按全部命中主题统计的主题覆盖数量 |
...
个股暴露页签上方的“主题覆盖统计”使用 `theme_names` 全部命中集合统计股票数量，不使用 `primary_theme` 做排他分类。`primary_theme` 只是表格快速浏览字段，因此像雅克科技、佰维存储这类同时命中半导体芯片和存储芯片的股票，会计入所有命中的主题股票池。
