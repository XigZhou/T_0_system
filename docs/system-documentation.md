...
- 信号条件、评分表达式和动量指标读取 `open/high/low/close`，这些字段是前复权 `qfq_*` 别名。
- 实盘账户回测的买入、卖出、现金、持仓市值、期末权益、年度稳定性、月度表现和个股贡献使用未复权除权价格：成交用 `raw_open`，截止日估值用 `raw_close`。
- 实盘账户回测的 `trade_rows.shares` 是真实成交股数：买入按 `lot_size` 向下取整，卖出沿用持仓真实股数；信号质量模式固定按 100 股虚拟样本计算。
- `adj_factor` 只参与前复权价格和指标构造，不会在交易流水、持仓市值或盈亏计算中把股数乘以复权因子比例。
...
信号质量回测用于先判断“买入条件、卖出条件、评分表达式本身是否有效”。它不模拟账户现金和仓位金额占用，但会按固定 100 股虚拟持仓去重，避免同一股票持仓期内连续重复买入。信号质量模式的 `daily_rows.equity` 是从 100000 起步、按每日完成信号平均收益率复利得到的“信号净值”，不是账户现金加持仓市值。
...
- 每个入选信号默认 `T+1` 开盘虚拟买入。
- 不使用 `initial_cash`、账户现金、`per_trade_budget` 和 `lot_size`；交易流水固定按 100 股真实股数计算买入净额、卖出净额和盈亏，不影响信号是否入选，也不随 `adj_factor` 调整股数。
...
| 字段 | 说明 |
| --- | --- |
| `summary` | 入选信号数、完成信号数、平均/中位单笔收益、胜率、收益因子、信号净值收益、回撤、持仓期跳过信号数；`result_mode=signal_quality`、`metric_basis=signal_return_diagnostic`、`auditable=false` |
...
| `contribution_rows` | 个股在信号样本中的收益率贡献，`total_signal_return` 是信号收益率累加，不是账户盈亏金额 |
...
- 提供表单化回测入口
- 展示组合结果摘要、条件诊断、排名质量、曲线与回撤、年度稳定性、月度表现、退出原因、信号明细、交易流水、个股贡献；信号质量模式展示信号净值/信号样本流水，实盘账户模式展示账户权益/真实交易流水
...
- 右上角导航包含 `股票池模板` 链接，可直接进入 `/stock-pools` 维护批量回测使用的股票池模板
...
- 年度稳定性：信号质量模式是年度信号净值稳定性，实盘账户模式是年度账户权益表现
- 月度表现：信号质量模式是月度信号净值表现，实盘账户模式是月度账户权益表现
- 退出原因：卖出条件触发与固定/最大持有退出的交易质量对比
- 期末持仓：显示截止日未平仓股票的市值、浮动盈亏、持有天数和计划退出状态
- 截止日卖出信号：显示结束日收盘触发但未使用未来开盘价成交的卖出提醒
- 每日信号明细：表头提供 `下载Excel`，导出当前参数重跑后的每日选股明细；生成时按钮显示 `生成中...`，失败原因显示在状态栏
- 交易流水：表头提供 `下载Excel`，导出当前参数重跑后的流水；信号质量模式标题为“信号样本流水”，实盘账户模式标题为“真实交易流水”
- 信号样本流水字段包含信号日期、交易日期、股票代码、股票名称、排名、评分、动作、价格、价格口径、股数、样本盈亏、交易收益、持有天数、退出类型和执行说明；股数固定为 100 股，不用于账户审计
- 真实交易流水字段包含交易日期、信号日期、股票代码、股票名称、排名、评分、动作、价格、价格口径、真实股数、成交金额、费用、净金额、交易后现金、盈亏、交易收益、持有天数、价差盈亏、退出原因和执行说明；股数满足 `lot_size` 整手约束，可用于审计
- 交易流水一行只表示一个买入或卖出动作；买入行盈亏为空，卖出行按卖出净额减买入净额计算盈亏；页面和单表 Excel 中金额、价格、费用、现金、权益、市值、盈亏类字段统一保留两位小数
- 价格口径：买卖成交使用未复权开盘价（除权价格），截止日持仓估值使用未复权收盘价，信号和评分指标使用前复权价格；复权因子不调整交易股数
...
- 用当天收盘后的股票池模板 SQLite 数据生成明日候选买入名单；旧 API 仍可用处理后 CSV 目录兼容调用
...
- 第一阶段保存模板和股票列表到 SQLite；第二阶段支持共享日线与指标入库、任务日志和补数脚本；第四阶段已先接入批量回测和每日收盘选股，其他旧模块后续再逐步切换。
...
- 旧接口仍支持将实盘账户回测结果打包为 ZIP 下载
- 前端默认使用单表 Excel 下载：每日选股明细和交易流水分别下载，避免用户只想核对一张表时还要解压

### 入口

- 单表 Excel API：`POST /api/run-backtest-table-export?mode=signal_quality|account&table=pick_rows|trade_rows`
- 旧 ZIP API：`POST /api/run-backtest-export`
- 前端按钮：`每日选股明细`、`信号样本流水` 或 `真实交易流水` 表头的 `下载Excel`；下载链接会挂到页面后再触发，兼容浏览器下载行为；旧“下载表格压缩包”按钮已隐藏
...
- `每日选股明细.xlsx`：导出 `pick_rows`
- `信号质量回测_信号样本流水.xlsx` / `实盘账户回测_真实交易流水.xlsx`：导出 `trade_rows`，工作簿包含 `口径说明` sheet；信号质量模式的流水不可用于账户审计，实盘账户模式的流水包含成交金额、费用、净金额、交易后现金、收益率和退出原因，金额类字段按两位小数写出
- `个股贡献汇总.csv`
- `条件诊断.csv`
- `年度稳定性.csv`
- `月度表现.csv`
