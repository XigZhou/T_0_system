...
- 交易流水：表头提供 `下载Excel`，导出当前参数重跑后的交易流水；生成时按钮显示 `生成中...`，失败原因显示在状态栏
- 交易流水字段固定为：交易日期、信号日期、股票代码、股票名称、排名、评分、动作、价格、价格口径、股数、盈亏、执行说明
- 交易流水一行只表示一个买入或卖出动作；买入行盈亏为空，卖出行按卖出净额减买入净额计算盈亏；页面和单表 Excel 中金额、价格、费用、现金、权益、市值、盈亏类字段统一保留两位小数
- 价格口径：买卖成交使用未复权开盘价（除权价格），信号和评分指标使用前复权价格
- 个股贡献汇总
...
- 前端按钮：`每日选股明细` 和 `交易流水` 表头的 `下载Excel`；下载链接会挂到页面后再触发，兼容浏览器下载行为；旧“下载表格压缩包”按钮已隐藏
...
- `交易流水.xlsx`：导出 `trade_rows`，字段固定为交易日期、信号日期、股票代码、股票名称、排名、评分、动作、价格、价格口径、股数、盈亏、执行说明；金额类字段按两位小数写出
- `个股贡献汇总.csv`
- `条件诊断.csv`
- `年度稳定性.csv`
- `月度表现.csv`
