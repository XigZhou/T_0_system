...
本文档说明多账户模拟交易系统的使用方式、数据来源和运维口径。当前版本已经全面升级为“账户模板 + 股票池模板 SQLite”模式：账户模板、订单、成交、持仓、资产和日志统一保存在 SQLite，股票范围、信号字段、本地成交价和收盘估值价统一来自股票池模板系统。

## 1. 设计目标

- 每个 SQLite 账户模板对应一个独立模拟账户；旧 YAML 只作为首次兼容导入来源。
...
- 使用 SQLite 账本表存储交易流水、持仓、资产和日志，按 `username + account_id` 隔离账户数据。
...
| 旧 YAML 导入目录 | `configs/paper_accounts/` |
...
`data_store/paper_trading.sqlite` 是多账户模拟交易的运行时账本库。旧 YAML 模板建议使用 UTF-8 编码；系统也兼容历史 Windows 模板常见的 GB18030 编码，并会在首次读取时导入 SQLite。

打开 `/paper` 时，页面会自动读取所选账户的 SQLite 账本，并展示待执行订单、成交流水、当前持仓、每日资产和运行日志。`/paper` 只负责模板选择、账本读取、订单生成、成交执行和持仓估值；模板字段编辑已经独立到 `/paper/templates` 页面。

## 3. 数据来源与字段定义

### 股票池模板

多账户模拟交易模板通过股票池字段选择股票池模板；旧 YAML 仍按下面结构兼容导入：
...
股票池用户由当前登录态自动带入，前端不提供手工输入用户名。旧 YAML 中的 `用户: admin` 仅作为历史模板导入兼容字段。
...
`/paper/templates` 不再显示模板目录、模板路径、股票池数据库、账本路径或日志目录。页面只展示账户编号、账户名称、资金、股票池模板、买卖条件、评分表达式、股数和费用等业务字段。

页面能力：

- `载入模板`：按当前用户和账户编号读取 SQLite 模板并填充表单。
- `新建模板`：初始化空白模板草稿。
- `复制模板`：复制当前表单为新草稿，自动生成新的账户编号和账户名称。
- `保存模板`：覆盖当前账户编号对应的 SQLite 模板。
- `另存为新模板`：创建新的 SQLite 账户模板。
- `删除模板`：停用 SQLite 模板，不删除历史账本记录。

保存规则：

- 同一用户下账户编号和账户名称不能重复。
- 覆盖保存不能切换账户编号；如需新账户请使用“另存为新模板”。
- 保存时会校验股票池模板存在；如果股票池模板不存在，后端返回错误。
- 账本路径、日志目录和股票池数据库路径由系统默认决定，不作为前端输入或冲突条件。
...
- `/single` 单股回测已默认读取股票池 SQLite 共享行情库，旧处理后 CSV/Excel 入口保留兼容。
- 多账户模拟交易已经全面切换为账户模板 SQLite 和股票池模板 SQLite，不再读取账户 YAML 中的旧 `处理后数据目录` 字段，也不再依赖旧 CSV 日线目录。
...
- 使用测试 SQLite 股票池生成、执行、卖出、估值和读取 SQLite 账本。
